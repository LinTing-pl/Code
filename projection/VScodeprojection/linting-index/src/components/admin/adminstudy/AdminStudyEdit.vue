<template>
  <div class="adminstudyedit-container">
    <div class="left">
      <div class="display">
        <div class="display-content" v-html="sectionContent"></div>
    ...
    <div class="right">
      ...
      <div class="bottom" ref="bottom">
        ...
        >
          ...
          <div :class="{ sections: true, active: activeIndex === index }">
            ...
            <div
              :class="{
                section: true,
                active: activeIndex === index && activeIndex1 === index1,
              }"
              v-for="(item1, index1) in item.sections"
              :key="index1"
              ref="section"
              @dblclick="modifyDom"
              @click="setIndex($event, index, index1)"
...
export default {
  ...
  methods: {
    ...
    async submitBook() {
      ...
      let book = JSON.parse(localStorage.getItem(`adminstudyedit${this.id}`));
    ...
    setIndex(e, index, index1) {
      ...
      this.sectionId = setTimeout(() => {
        ...
        sessionStorage.setItem("adminstudyEdit" + this.id + "Index", index);
        sessionStorage.setItem("adminstudyEdit" + this.id + "Index1", index1);
      }, 250);
    },
  },
};
...
.adminstudyedit-container .left {
  width: 77%;
  display: flex;
  flex-direction: column;
}
.adminstudyedit-container .right {
  width: 23%;
  height: 606px;
  padding: 5px;
}
...
.left .display .display-btns .saveSection {
  height: 28px;
  margin-right: 10px;
...
.left .display .display-btns .saveSection:hover {
  background-color: #66b1ff;
}
.left .display .display-btns .submitBook {
  height: 28px;
  margin-right: 10px;
...
.right .bottom .chapter .sections .section.active,
.right .bottom .chapter .sections .section:hover {
  background-color: #409eff;
  box-shadow: 0 0 5px #409eff88;
}
.right .bottom .chapter .sections .section:hover {
  text-shadow: 0 0 4px #fff;
}
