<template>
  <div class="videocontent-container">
    ...
    <div class="videocard" v-for="(item, index) in cardList" :key="index">
      <div class="img">
...
export default {
  ...
  created() {
    let data = JSON.parse(sessionStorage.getItem("adminVideo"));
    this.currentPage =
      JSON.parse(sessionStorage.getItem("currAdminVideoPage")) || 1;
    if (data) {
      this.allList = data;
  ...
  methods: {
    async getData(flag) {
      await this.$axios.default.get("/dev-api/video/get").then((res) => {
        ...
        this.prioritiesCopy = { ...this.priorities };
        sessionStorage.setItem("adminVideo", JSON.stringify(res.data));
      });
    },
    handleCurrentChange() {
      sessionStorage.setItem("currAdminVideoPage", this.currentPage.toString());
      this.cardList = this.allList.slice(
    ...
    toAdd() {
      this.$router.push({ name: "adminvideoadd" });
    },
    edit(id) {
    ...
    del(id) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$axios.default
            .delete(`/dev-api/video/delete/${id}`)
            .then((res) => {
              this.$message({
                type: "success",
                message: "删除成功!",
...
.videocard .info .info-title {
  margin-bottom: 5px;
  font-weight: bold;
  font-size: 18px;
  overflow: hidden;
}
.videocard .info .info-info {
  text-align: left;
  line-height: 1.5;
  overflow: hidden;
}
...
.videocard .opts .diary-btns {
  display: flex;
  flex-direction: column;
}
.videocard .opts .diary-btns button {
  width: 99px;
  height: 48px;
  margin: 1px 1px;
  outline: none;
...
.videocard .opts .diary-btns .edit-btn {
  border: 1px solid #409eff;
  background: #409eff;
  color: #fff;
}
.videocard .opts .diary-btns .edit-btn:hover {
  background: #66b1ff;
}
.videocard .opts .diary-btns .del-btn {
  border: 1px solid #f56c6c;
  background: #f56c6c;
  color: #fff;
}
.videocard .opts .diary-btns .del-btn:hover {
  background: #f78989;
}
::v-deep.videocard .el-input-number {
  width: 120px;
}
.videocontent-container .videocard:nth-last-child(2) {
