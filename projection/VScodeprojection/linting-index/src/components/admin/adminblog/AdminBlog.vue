<template>
  <div class="blogcontent-container">
    ...
    <div class="blogcard" v-for="(item, index) in cardList" :key="index">
      <div class="img">
...
export default {
  ...
  created() {
    let data = JSON.parse(sessionStorage.getItem("adminBlog"));
    this.currentPage =
      JSON.parse(sessionStorage.getItem("currAdminBlogPage")) || 1;
    if (data) {
      this.allList = data;
  ...
  methods: {
    async getData(flag) {
      await this.$axios.default.get("/dev-api/blog/get").then((res) => {
        ...
        this.prioritiesCopy = { ...this.priorities };
        sessionStorage.setItem("adminBlog", JSON.stringify(res.data));
      });
    },
    handleCurrentChange() {
      sessionStorage.setItem("currAdminBlogPage", this.currentPage.toString());
      this.cardList = this.allList.slice(
    ...
    toAdd() {
      this.$router.push({ name: "adminblogadd" });
    },
    edit(id) {
...
.blogcard .info .info-title {
  margin-bottom: 5px;
  font-weight: bold;
  font-size: 18px;
  overflow: hidden;
}
.blogcard .info .info-info {
  text-align: left;
  line-height: 1.5;
  overflow: hidden;
}
...
.blogcard .opts .diary-btns {
  display: flex;
  flex-direction: column;
}
.blogcard .opts .diary-btns button {
  width: 99px;
  height: 48px;
  margin: 1px 1px;
  outline: none;
...
.blogcard .opts .diary-btns .edit-btn {
  border: 1px solid #409eff;
  background: #409eff;
  color: #fff;
}
.blogcard .opts .diary-btns .edit-btn:hover {
  background: #66b1ff;
}
.blogcard .opts .diary-btns .del-btn {
  border: 1px solid #f56c6c;
  background: #f56c6c;
  color: #fff;
}
.blogcard .opts .diary-btns .del-btn:hover {
  background: #f78989;
}
::v-deep.blogcard .el-input-number {
  width: 120px;
}
.blogcontent-container .blogcard:nth-last-child(2) {
