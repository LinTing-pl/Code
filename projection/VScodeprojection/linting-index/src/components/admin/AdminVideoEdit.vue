    ...
    <div class="right">
      ...
      <div class="middle">
        <label class="img" for="videoImg">
          ...
          <img ref="img" v-if="data.img" :src="data.img" alt="封面" />
        </label>
        <input
          type="file"
          class="videoImg"
...
export default {
  ...
  created() {
    ...
    if (this.activeIndex) {
      this.sectionContent = this.data.sections[this.activeIndex]?.mp4;
    }
  ...
  methods: {
    beforeUnload(e) {
      if (
        ...
        JSON.stringify(this.data) !==
          localStorage.getItem("adminvideoedit" + this.id)
      ) {
        e = e || window.event;
        if (e) {
          e.returnValue = "关闭提示";
    ...
    async submitVideo() {
      ...
      this.$axios.default
        .post("/dev-api/video/update", { video: this.data })
        .then((res) => {
          this.$message({
            type: "success",
            message: "提交成功!",
    ...
    setIndex(e, index) {
      ...
      this.chapterId = setTimeout(() => {
        ...
        this.sectionContent = this.data.sections[index].mp4;
        sessionStorage.setItem("adminvideoEdit" + this.id + "Index", index);
      }, 250);
    },
    pushVideo(e) {
      let file = e.target.files[0];
      ...
      //     _this.sectionContent = e.target.result;
      //   };
    },
    delVideo() {
      let i = JSON.parse(
      ...
      if (i !== null) {
        this.$confirm(
          ...
          .then(() => {
            ...
            sessionStorage.removeItem("adminvideoEdit" + this.id + "Index");
