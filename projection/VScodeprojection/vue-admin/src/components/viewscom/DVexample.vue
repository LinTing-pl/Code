...
import "https://cdn.staticfile.org/jquery/3.3.1/jquery.min.js";
import jsonurl from "../../utils/html/json.json?url";
const panel = ref();
onMounted(() => {
	var myChart = echarts.init(panel.value);
	var uploadedDataURL = jsonurl;
	//线条聚点
	var geoGpsMap = {
		1: [116.415, 39.915],
		2: [116.448, 39.935],
	...
	$(function () {
		...
		var lineData1 = [
			[
				10, 12, 13, 12, 14, 14, 16, 17, 18, 20, 22, 26, 27, 26, 23, 20,
				18, 18, 16, 16, 14, 13, 12, 10,
			],
			[
				68, 67, 66, 63, 61, 55, 53, 48, 46, 44, 42, 42, 36, 34, 36, 37,
				38, 42, 46, 44, 58, 62, 66, 68,
			],
			[
				1, 1, 1, 1, 1, 1, 1, 2, 3, 2, 1, 1, 1, 1, 1, 1, 3, 3, 1, 3, 3,
				1, 1, 1,
			],
			[
				56, 59, 60, 62, 56, 50, 40, 42, 38, 38, 39, 38, 40, 42, 38, 37,
				34, 35, 38, 44, 46, 50, 54, 56,
			],
			[
				1, 1, 2, 1, 5, 10, 20, 24, 30, 32, 32, 34, 38, 40, 40, 38, 32,
				25, 32, 32, 5, 1, 1, 1,
			],
		];
		var lineData2 = [
			[
				11, 12, 14, 12, 15, 13, 16, 16, 16, 21, 22, 24, 22, 24, 23, 21,
				19, 18, 15, 14, 15, 13, 12, 11,
			],
			[
				62, 64, 66, 65, 62, 58, 54, 49, 45, 44, 41, 40, 32, 36, 36, 39,
				38, 40, 46, 48, 52, 60, 63, 61,
			],
			[
				1, 1, 1, 2, 1, 1, 1, 2, 1, 2, 1, 1, 3, 1, 1, 1, 3, 2, 1, 1, 3,
				1, 2, 1,
			],
			[
				58, 59, 62, 62, 54, 50, 44, 42, 34, 33, 35, 38, 40, 44, 38, 35,
				32, 35, 33, 40, 46, 53, 56, 54,
			],
			[
				1, 1, 1, 1, 6, 12, 20, 22, 30, 33, 32, 35, 38, 42, 40, 37, 34,
				28, 32, 33, 10, 1, 1, 1,
			],
		];
		...
		for (var i = 0; i < mapData.length; i++) {
			...
			for (var j = 0; j < mapData[i].length; j++) {
				...
				if (
					mapData[i][j].value < li[i] ||
					mapData[i][j].value > zhu[i]
				) {
					count3[i]++;
		...
		$.getJSON(uploadedDataURL, function (geoJson) {
			...
			for (var n = 0; n < year.length; n++) {
				(optionXyMap01.options as any).push({
					//背景色
					...
					series: [
						...
						},
						{
							...
							//调用聚点随机
							data: convertToLineData001(
								line4,
								geoGpsMap[Math.ceil(4)]
							),
						},
						//柱状图
						{
...
<template>
	<div class="example-container">
		<div ref="panel" id="chart-panel"></div>
	</div>
</template>
