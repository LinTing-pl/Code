...
#  0 <= special[i][j] <= 50 
#  
#  Related Topics 位运算 记忆化搜索 数组 动态规划 回溯 状态压缩 👍 327 👎 0
...
class Solution(object):
    def shoppingOffers(self, price, special, needs):
        ...
        :rtype: int
        """
        return min([sum(p * n for p, n in zip(price, needs))] + [
            sp[-1] + self.shoppingOffers(price, special, [n - c for n, c in zip(needs, sp[:-1])]) for sp in special if
            not any(n < o for n, o in zip(needs, sp))])
