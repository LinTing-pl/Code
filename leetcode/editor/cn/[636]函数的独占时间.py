...
class Solution(object):
    def exclusiveTime(self, n, logs):
        ...
        stack = []
        for log in logs:
            f, flag, time = log.split(":")
            f = int(f)
            time = int(time)
            if flag == "start":
                if stack:
                    # 栈顶函数暂停, 记下它已经运行的时间
                    pre_f, start_time = stack[-1]
                    res[pre_f] += time - start_time
                    # 当前函数入栈
                stack.append((f, time))
            elif flag == "end":
                # 栈顶函数结束
                f, start_time = stack.pop()
                res[f] += time - start_time + 1
                if stack:
                    # 之前有函数还未结束（之前运行的时间已经算过）
                    # 更新它的开始时间到当前时间+1
                    pre_f, start_time = stack.pop()
                    stack.append((pre_f, time + 1))
        return res
